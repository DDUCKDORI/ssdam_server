...
public class AppleLoginController {
    ...
    @ResponseBody
    public ResponseEntity<LoginDTO> appleCallBack(AppleDTO appleDTO) throws ParseException, IOException, JOSEException, net.minidev.json.parser.ParseException {
        //code
        //id_token
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));
        //여기서 sub 값으로 데이터베이스에 해당 sub 값이 있는지 판단
        LoginDTO loginDTO = appleLoginService.getToken(appleDTO);
        //있다면 refresh_token이랑 사용자 정보반환
        if(loginDTO!=null){
            //DB에 sub값이 있다면
            //refresh_token으로 access_token 재발급 받고
            // loginDTO 에 담아 반환.
            return new ResponseEntity<>(loginDTO,httpHeaders,HttpStatus.OK);
        }
        //없다면 프로세스 진행 -> sub랑 Refresh_token 저장 -> Okay
        loginDTO = appleLoginService.authToken(appleDTO);
        return new ResponseEntity<>(loginDTO, httpHeaders, HttpStatus.OK);
    }
    ...
    public ResponseEntity<ResponseDTO> join(@RequestBody LoginDTO loginDTO){
        ...
        HttpHeaders httpHeaders = new HttpHeaders();
        httpHeaders.setContentType(new MediaType("application", "json", Charset.forName("UTF-8")));
        ResponseDTO responseDTO = appleLoginService.joinMember(loginDTO);
